...
export default function ContractsPage() {
  ...
  const [form, setForm] = useState({ title: '', sponsor_id: '', value_nok: '', season: '2025-2026', start_date: '', end_date: '' })
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      const [contractsRes, sponsorsRes] = await Promise.all([
        supabase.from('contracts').select('id, title, value_nok, season, status, start_date, end_date, sponsors(company_name)').eq('org_id', userData.org_id).order('created_at', { ascending: false }),
  ...
  async function handleSave() {
    ...
    const { data, error: saveError } = await supabase.from('contracts').insert({
      ...
      season: form.season, start_date: form.start_date || null, end_date: form.end_date || null, status: 'active',
    }).select('id, title, value_nok, season, status, start_date, end_date, sponsors(company_name)').single()
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {showForm && (
          <div className="card mb-8">
            ...
            {sponsors.length === 0 && <div className="bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-lg px-4 py-3 mb-6">Add a sponsor first. <Link href="/dashboard/sponsors" className="underline">Add a sponsor</Link></div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <div><label className="label">Total value (NOK)</label><input type="number" className="input" placeholder="50000" value={form.value_nok} onChange={e => update('value_nok', e.target.value)} /></div>
                ...
                <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedContract(expandedContract === contract.id ? null : contract.id)}>
                  <div>
                    ...
                    <p className="text-sporr-muted text-sm mt-0.5">
                      {contract.sponsors?.company_name}{contract.season && <span> · {contract.season}</span>}
                      <span> · {obligations[contract.id]?.length || 0} obligation{obligations[contract.id]?.length !== 1 ? 's' : ''}</span>
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    {contract.value_nok > 0 && <p className="text-sporr-dark font-medium text-sm">Kr {contract.value_nok.toLocaleString()}</p>}
